...
function Actions({ post }) {
  ...
  if (!user)
    return showToast(
      "Error",
      "You must be logged in to your account to like a post",
      "error"
    );
  ...
  const handleReply = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.error) {
        showToast({ ...post, replies: [...post.replies, data.reply] });
      }
      const updatedPosts = posts.map((p) => {
        if(p._id === post._id) {
  ...
  return (
    <Flex gap={5} onClick={(e) => e.preventDefault()}>
